...
/**
 * Adjust the gamma of the currently displayed tile.
 * 
 * @author Hj. Malthaner
 */
public class AdjustGamma extends YUVPaintingToolBase implements ValueChangeInterface
{
        ...
        for(int y=0; y<canvas.getHeight(); y++)
        {
            for(int x=0; x<canvas.getWidth(); x++)
            {
                ...
                if((rgb & 0xFF000000) == 0xFF000000)
                {
                    ...
                    final double Y = Math.pow(yuv[0] / 255.0, gamma);
                    yuv[0] = 255.0 * Y;
    ...
    /**
     * Gamma value slider was dragged.
     * @param values New selected value.
     */
    @Override
    public void onValueChanged(int [] values)
    {
        // Hajo: User expects lower values to be more dark
        gamma = 1.0 - ((values[0] - 100) / 100.0);
