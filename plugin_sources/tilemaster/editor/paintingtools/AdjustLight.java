...
/**
 * Adjust the lightness of the currently displayed tile.
 * 
 * @author Hj. Malthaner
 */
public class AdjustLight extends YUVPaintingToolBase implements ValueChangeInterface
{
    ...
    private void showUI()
    {
        ...
        requester.askValues(editor, 
                new String [] {"White Point:", "Black Point:"},
                0, 100, 50, this);
    }
        ...
        for(int y=0; y<canvas.getHeight(); y++)
        {
            for(int x=0; x<canvas.getWidth(); x++)
            {
                ...
                if((rgb & 0xFF000000) == 0xFF000000)
                {
                    ...
                    final double newWhite = 1.0 + (white/50.0);
                    final double newBlack = 0.0 + (black/50.0);
                    
                    // Y in [0.0 .. 1.0] 

                    final double Y = yuv[0] / 255.0;
                    yuv[0] = 255.0 * (newBlack + Y * (newWhite - newBlack));
    ...
    public void onValueChanged(int [] values)
    {
        ...
        // System.err.println("White=" + white + " black=" + black);
